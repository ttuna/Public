...
public:
    ...
    template<typename T> static const QVariantMap Get(const T& in_obj, const bool in_recursive = false)
    {
        ...
        prop_map = GetPropertiesVariantMap(&in_obj, meta_object, in_recursive);
...
private:
    ...
    // GetProperty
    // ----------------------------------------------------------------------------
    static QVariantMap GetPropertiesVariantMap(const QObject* in_obj, const QMetaObject* in_meta_obj, const bool in_recursive)
    {
        ...
        for (int i=0; i<prop_count; ++i)
        {
            ...
            if (meta_prop.type() >= QVariant::UserType && in_recursive == true)
            {
                ...
                QVariantMap sub_map;
                sub_map = GetPropertiesVariantMap(sub_obj, sub_meta_object, in_recursive);
